...
export function VocabularyEntryCard({ entry, onToggleFavorite, onRemove }: VocabularyEntryCardProps) {
  ...
  return (
    <div className="flex items-start justify-between gap-3 py-3.5">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">{entry.meaning}</p>
        {entry.example && (
          <p lang="ar" dir="rtl" className="font-arabic mt-1.5 text-left text-sm text-muted-foreground/80">
            {entry.example}
          </p>
        )}
